...
userRouter.get("/search/:page/:number", async (req, res) => {
  ...
  try {
    ...
    if (query.age) {
      if (query.age === "<10") query.age = { $lt: 10 };
      if (query.age === "<20") query.age = { $lte: 20, $gte: 10 };
      if (query.age === "<30") query.age = { $lte: 30, $gte: 21 };
      if (query.age === "<40") query.age = { $lte: 40, $gte: 31 };
      if (query.age === ">40") query.age = { $gt: 40 };
    }
...
userRouter.post("/report", (req, res) => {
...
userRouter.get("/single/:unique", async (req, res) => {
  let unique = decodeURIComponent(req.params.unique);
  unique = unique.replace(/\D/g, "");

  try {
    let user = await userModel.findOne({ unique }).lean();
    if (!user) {
